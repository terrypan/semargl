 ...
 * </p>
 */
public final class RdfaXPathParser extends Pipe<TripleSink> implements SaxSink, TripleSink, ProcessorGraphHandler, IParser {
    ...
    private RdfaXPathParser(TripleSink sink) {
        super(sink);
    ...
    public static SaxSink connect(TripleSink sink) {
        return new RdfaXPathParser(sink);
    }

    @Override
    public void startDocument() {
        //System.out.println("[RDFa XPATH StartDoc]");
    ...
    private void processRoleAttribute(String id, String roleVal, EvalContext current) {
        ...
        while (splitter.hasNext()) {
            try {
                ...
                addNonLiteral(subject, XHTML_VOCAB + "role", role, xpath);
        ...
        if (newSubject != null) {
            ...
            while (splitter.hasNext()) {
                try {
                    ...
                    addNonLiteral(newSubject, RDF.TYPE, iri, xpath);
                } catch (MalformedIriException e) {
                    // do nothing
                }
            }
        ...
        if (rels != null) {
            ...
            for (String predicate : rels) {
                ...
                if (inList) {
                    ...
                    if (current.object != null) {
                        addNonLiteral(current.subject, iri, current.object, xpath);
                    } else {
        ...
        if (revs != null) {
            for (String predicate : revs) {
                ...
                try {
                    ...
                    if (current.object != null) {
                        addNonLiteral(current.object, iri, current.subject, xpath);
                    } else {
        ...
        while (splitter.hasNext()) {
            ...
            if (content != null || langOrUri != null) {
                if (dh.rdfaVersion > RDFa.VERSION_10 && inList) {
                    ...
                    if (content != null) {
                        addLiteralTriple(current.subject, iri, content, langOrUri, xpath);
                    } else {
                        addNonLiteral(current.subject, iri, langOrUri, xpath);
                    }
                }
    ...
    private void processIncompleteTriples(EvalContext current, EvalContext parent) {
        ...
        for (Iterator iti = parent.incomplTriples.iterator(); iti.hasNext(); ) {
            ...
            if (predicateOrList == REVERSED_TRIPLE_FLAG) {
                addNonLiteral(current.subject, (String) iti.next(), subject, xpath);
            } else if (predicateOrList instanceof String) {
                addNonLiteral(subject, (String) predicateOrList, current.subject, xpath);
            } else {
        ...
        if (current.parsingLiteral && xmlString != null) {
            ...
            if (dh.rdfaVersion == RDFa.VERSION_10 && content.indexOf('<') == -1) {
                for (String pred : xmlStringPred) {
                    addPlainLiteral(xmlStringSubj, pred, content, current.lang, xpath);
                }
            } else {
                for (String pred : xmlStringPred) {
                    addTypedLiteral(xmlStringSubj, pred, content, RDF.XML_LITERAL, xpath);
                }
            }
        }
    }
        ...
        if (!parent.parsingLiteral && parent.objectLit != null) {
            ...
            if (inlist) {
                ...
                for (String predIri : current.properties) {
                    ...
                    addLiteralTriple(current.subject, predIri, content, dt, xpath);
                }
            }
        }
    }
        ...
        for (String pred : list.keySet()) {
            ...
            for (Iterator<String> iterator = list.get(pred).iterator(); iterator.hasNext(); ) {
                ...
                if (res == LITERAL_OBJECT_FLAG) {
                    ...
                    addLiteralTriple(child, RDF.FIRST, content, langOrDt, xpath);
                } else {
                    addNonLiteral(child, RDF.FIRST, res, xpath);
                }
                if (prev == null) {
                    start = child;
                } else {
                    addNonLiteral(prev, RDF.REST, child, xpath);
                }
                prev = child;
            }
            if (start == null) {
                addNonLiteral(current.subject, pred, RDF.NIL, xpath);
            } else {
                addNonLiteral(prev, RDF.REST, RDF.NIL, xpath);
                addNonLiteral(current.subject, pred, start, xpath);
            }
        }
        list.clear();
    }
    ...
    public Vocabulary loadVocabulary(String vocabUrl) {
        if (sinkOutputGraph) {
            sink.addNonLiteral(dh.base, RDFa.USES_VOCABULARY, vocabUrl, xpath);
        }
    ...
    private void addProcessorGraphRecord(String recordClass, String recordContext) {
        if (dh.rdfaVersion > RDFa.VERSION_10 && sinkProcessorGraph) {
            ...
            sink.addNonLiteral(errorNode, RDF.TYPE, recordClass, xpath);
            sink.addPlainLiteral(errorNode, RDFa.CONTEXT, recordContext + location, null, xpath);
        }
    }
    ...
    private void addLiteralTriple(String subject, String pred, String content, String langOrDt, String xpath) {
        if (langOrDt == null || langOrDt.length() < 6 || langOrDt.indexOf(':') == -1) {
            addPlainLiteral(subject, pred, content, langOrDt, xpath);
        } else {
            addTypedLiteral(subject, pred, content, langOrDt, xpath);
        }
    }
    ...
    public void addNonLiteral(String subj, String pred, String obj, String xpath) {
        ...
        if (!expandVocab) {
            sink.addNonLiteral(subj, pred, obj, xpath);
            return;
        }
        addNonLiteralWithObjExpansion(subj, pred, obj, xpath);
        for (String predSynonym : contextStack.peek().expand(pred)) {
            addNonLiteralWithObjExpansion(subj, predSynonym, obj, xpath);
        }
        
    }
    
    private void addNonLiteralWithObjExpansion(String subj, String pred, String obj, String xpath) {
        ...
        for (String objSynonym : contextStack.peek().expand(obj)) {
            sink.addNonLiteral(subj, pred, objSynonym, xpath);
        }
    }
    ...
    public void addPlainLiteral(String subj, String pred, String content,
        ...
        sink.addPlainLiteral(subj, pred, content, lang, xpath);
        for (String predSynonym : contextStack.peek().expand(pred)) {
            sink.addPlainLiteral(subj, predSynonym, content, lang, xpath);
        }
    }
    ...
    public void addTypedLiteral(String subj, String pred, String content,
        ...
        sink.addTypedLiteral(subj, pred, content, type, xpath);
        for (String predSynonym : contextStack.peek().expand(pred)) {
            sink.addTypedLiteral(subj, predSynonym, content, type, xpath);
        }
